...
def _row(content):
    return {"ledger_fingerprint": "ledger", "disposition": "recover_car19", "http_status": 200,
            "source_key": "html/key", "sha256": hashlib.sha256(content).hexdigest(),
            "listing_id": "listing", "fetched_at": "2026-04-21T01:02:03+00:00", "artifact_id": 9,
...
def test_process_row_is_dry_run_by_default_and_validates_identity(monkeypatch):
    ...
    monkeypatch.setattr(backfill, "parse_cars_detail_page_html_v1", lambda *_args: ({"listing_id": "listing", "listing_state": "active"}, [], {}))
