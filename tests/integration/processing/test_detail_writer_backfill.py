...
def _hot_snapshot(vc, listing_id, vin):
    vc.execute(
        "SELECT listing_id, vin, price, make, model, last_seen_at, last_artifact_id "
        "FROM ops.price_observations WHERE listing_id = %s::uuid",
        (listing_id,),
    )
    price = dict(vc.fetchone())
    vc.execute(
        "SELECT vin, listing_id, mapped_at, artifact_id FROM ops.vin_to_listing WHERE vin = %s",
        (vin,),
    )
    mapping = dict(vc.fetchone())
    vc.execute(
        "SELECT listing_id, num_of_attempts FROM ops.blocked_cooldown WHERE listing_id = %s::uuid",
        (listing_id,),
    )
    cooldown = dict(vc.fetchone())
    vc.execute(
        "SELECT listing_id, status FROM ops.detail_scrape_claims WHERE listing_id = %s::uuid",
        (listing_id,),
    )
    ...
    vc.execute(
        "SELECT * FROM ops.ops_detail_scrape_queue WHERE listing_id = %s::uuid", (listing_id,)
    )
    queue = vc.fetchone()
...
def test_backfill_leaves_hot_rows_unchanged_and_dates_event_historically(
    vc,
    seed_artifact_c,
    seed_price_observation_c,
    seed_vin_to_listing_c,
    seed_detail_claim_c,
    state,
    event_type,
):
    ...
    vc.execute(
        "INSERT INTO ops.blocked_cooldown (listing_id, num_of_attempts) VALUES (%s::uuid, 3)",
        (listing_id,),
    )
    ...
    if state == "active":
        ...
        result = write_detail_unlisted(
            _primary(listing_id, vin, state=state),
            artifact["artifact_id"],
            APRIL,
            listing_id,
            None,
    ...
    vc.execute(
        "SELECT event_type, event_at FROM staging.price_observation_events "
    ...
    vc.execute(
        "DELETE FROM staging.silver_observations WHERE artifact_id = %s", (artifact["artifact_id"],)
    )
    vc.execute(
        "DELETE FROM staging.price_observation_events WHERE listing_id = %s::uuid", (listing_id,)
    )
