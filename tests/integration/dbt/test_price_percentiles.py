...
def _seed_price_data(dbt_cur):
    ...
    dbt_cur.execute("""
                    INSERT INTO public.srp_observations 
                        (id, artifact_id, listing_id, created_at, fetched_at,
                         make, model, trim, price, search_scope, vin)
                    VALUES
                        (1, 1, 'L1', now() - interval '1 day', now() - interval '1 day',
                            'Test-Make', 'Test-Model', 'Test-Trim', 10000, 'national',
                            'SRPL1000000010000'),
                        (2, 2, 'L2', now() - interval '1 day', now() - interval '1 day',
                            'Test-Make', 'Test-Model', 'Test-Trim', 20000, 'national',
                            'SRPL2000000020000'),
                        (3, 3, 'L3', now() - interval '1 day', now() - interval '1 day',
                            'Test-Make', 'Test-Model', 'Test-Trim', 30000, 'national',
                            'SRPL3000000030000'),
                        (4, 4, 'L4', now() - interval '2 days', now() - interval '2 days',
                            'Test-Make', 'Test-Model', 'Test-Trim', 40000, 'national',
                            'SRPL4000000040000'),
                        (5, 5, 'L5', now() - interval '2 days', now() - interval '2 days',
                            'Test-Make', 'Test-Model', 'Test-Trim', 50000, 'national',
                            'SRPL5000000050000'),
                        (6, 6, 'L6', now() - interval '4 days', now() - interval '4 days',
                            'Test-Make', 'Test-Model', 'Test-Trim', 15000, 'national',
                            'SRPL6000000015000'),
                        (7, 7, 'L7', now() - interval '4 days', now() - interval '4 days',
...
def seed_and_build(dbt_conn, run_dbt):
    ...
    with dbt_conn.cursor() as cur:
        cur.execute("""
            TRUNCATE public.raw_artifacts, public.srp_observations
        """)
