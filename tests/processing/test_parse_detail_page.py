"""Unit tests for processing/processors/parse_detail_page.py.

Ported from tests/scraper/processors/test_parse_detail_page.py.
...
class TestPrimaryJsonExtraction:
    ...
    def test_empty_script_element_returns_none_for_listing_id(self):
        html = '<script id="initial-activity-data" type="application/json">   </script>'
...
class TestDealerCardParsing:
    ...
    def test_dealer_website_strips_utm(self):
        html = (
            _activity_script({"listing_id": "a"})
            + '<div class="dealer-card"><div class="website">'
            '<a href="https://dealer.com?utm_source=cars">site</a></div></div>'
        )
        primary, _, _ = parse_cars_detail_page_html_v1(html)
...
class TestCarouselParsing:
    def test_carousel_found_flag_true(self):
        html = ACTIVE_DETAIL_HTML + '<div class="listings-carousel"></div>'
    ...
    def test_carousel_missing_body_counted(self):
        carousel_html = """
        <div class="listings-carousel">
          <fuse-card>
            <fuse-save data-listing-id="bd-0000-0000-0000-000000000001"></fuse-save>
            <span class="price">$20,000</span>
          </fuse-card>
        </div>"""
        _, _, meta = parse_cars_detail_page_html_v1(ACTIVE_DETAIL_HTML + carousel_html)
        assert meta.get("missing_body", 0) >= 1

    def test_carousel_missing_mileage_counted(self):
        carousel_html = """
        <div class="listings-carousel">
          <fuse-card>
            <fuse-save data-listing-id="ml-0000-0000-0000-000000000001"></fuse-save>
            <span class="price">$20,000</span>
            <span class="body">New 2024 Honda CR-V EX</span>
          </fuse-card>
        </div>"""
        _, _, meta = parse_cars_detail_page_html_v1(ACTIVE_DETAIL_HTML + carousel_html)
        assert meta.get("missing_mileage", 0) >= 1
...
class TestWriterContract:
    """Assert all fields that detail_writer reads from primary are present."""

    def test_all_writer_fields_present(self):
        primary, _, _ = parse_cars_detail_page_html_v1(ACTIVE_DETAIL_HTML)
        missing = WRITER_PRIMARY_FIELDS - primary.keys()
        assert missing == set(), f"Primary missing writer-consumed fields: {missing}"
    ...
    def test_listing_id_from_href_when_no_data_attribute(self):
        ...
        <div class="listings-carousel">
          <fuse-card-carousel>
            <fuse-card>
              <fuse-save></fuse-save>
              <a href="/vehicledetail/{uid}/">Details</a>
              <span class="price">$25,000</span>
        ...
        """
        soup = BeautifulSoup(html, "lxml")
        cards, _ = _parse_carousel_cards(soup)
        assert len(cards) == 1
        assert cards[0]["listing_id"] == uid
