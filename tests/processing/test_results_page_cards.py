"""Unit tests for processing/processors/results_page_cards.py.

Ported from tests/scraper/processors/test_results_page_cards.py.
Contract test updated from n8n fields to writer fields — what
processing/writers/srp_writer.py actually reads from each listing.
"""
import json

from processing.processors.results_page_cards import (
    _digits_to_int,
    parse_cars_results_page_html_v3,
)

# Fields that srp_writer reads from each listing dict
WRITER_LISTING_FIELDS = {
    "listing_id",
...
class TestV3FieldMapping:
    def test_canonical_url_built_from_listing_id(self):
        ...
        listings, _ = parse_cars_results_page_html_v3(html)
        assert listings[0]["canonical_detail_url"] == (
            "https://www.cars.com/vehicledetail/xyz-0000-0000-0000-000000000099/"
        )
    ...
    def test_isa_context_none_when_missing(self):
        data = {"listingId": "isa-0000-0000-0000-000000000001", "make": "Honda"}
        html = f"<spark-card data-vehicle-details='{json.dumps(data)}'></spark-card>"
        listings, _ = parse_cars_results_page_html_v3(html)
...
class TestV3Meta:
    def test_parser_name(self):
        _, meta = parse_cars_results_page_html_v3("<html></html>")
...
class TestWriterContract:
    """Assert all fields that srp_writer reads are present in listing output."""

    def test_all_writer_fields_present(self):
        html = _make_v3_card("spark-card")
        listings, _ = parse_cars_results_page_html_v3(html)
        assert len(listings) == 1
        missing = WRITER_LISTING_FIELDS - listings[0].keys()
        assert missing == set(), f"Listing missing writer-consumed fields: {missing}"
...
class TestDigitsToInt:
    ...
    def test_non_scalar_returns_none(self):
        assert _digits_to_int([1, 2, 3]) is None
        assert _digits_to_int({"a": 1}) is None
