...
--
-- The legacy last_detail_scraped_at column was dropped by V049 (Plan 147
-- Stage 4). Through the dual-write release this statement bound it to the
-- same parameter as last_detail_enriched_at, so the two could not disagree;
-- that is what made the drop safe rather than hopeful.
--
