...
| Q3 | Should MLflow be **exposed behind Caddy** (`/mlflow`) now, or stay internal-only? | **Internal-only first; authenticated `/mlflow` exposure is a final Gate B step.** Keep MLflow aligned with the existing `cartracker.info/<service_name>` ingress convention rather than introducing a one-off subdomain. Add a Caddy `/mlflow/*` route + the same auth posture used by other internal/admin UIs after the provenance bridge and first backtest runs prove the service is worth keeping always-on. If MLflow has subpath/static-asset issues, treat that as a broader ingress-design decision for all services, not an MLflow-only exception. |
