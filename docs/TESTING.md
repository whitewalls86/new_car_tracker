...
|---|---|---|
| Every module-level SQL statement in a production module is executed by a Layer 2 test | ~~Inline SQL at an `.execute()` call site is the opposite direction and is measured by nothing~~ — **mechanised and closed 2026-09-01 by Plan 162 Stage 7.** [G5](#the-gap-list) is asserted by `test_no_production_module_holds_sql_at_its_execute_call_site`, and `INLINE_SQL_WAIVERS` is `()` — seeded at the 66 measured sites and drained the same day, each by the extraction it was waiting for. What stays unasserted is Spark: a SQL *fragment* (`selectExpr`, `expr`, a string `filter`) leads with no verb, and the DataFrame API is not text at all, so neither is visible to a static read | Plan 162 |
