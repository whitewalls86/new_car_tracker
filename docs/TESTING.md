...
| G30 | **A rule can exist unregistered, unproved and broken, and the suite stays green.** Found 2026-09-09 by Plan 162 Stage Q, which wrote four rules and reported 3,899 passed with all four unregistered and one of them defective: `test_every_heavy_job_starts_the_compose_services` joined a job's every `run:` step into one string, so the Flyway step's mention of the override file satisfied it with the `up` step deleted. The obligation reads this table's `Asserted by` column, so a rule in no row owes no mutation, and `test_every_asserted_rule_names_a_real_test` declines the reverse direction by design — the curated exception list it was declined over is **7 items**, four of which already carry mutations. **Shape cannot supply the missing definition**: "a test that asserts about the repository" would scope 411 of the 421 definitions at the top level of `tests/`, sweeping in 132 observability-config and 43 planning-docs tests | Not asserted. The repair is Plan 162 Stage AG: rules move to a directory, membership is read off the filesystem rather than a table, and every test there owes a row and a mutation — seeded at ~83 | Plan 162 |
