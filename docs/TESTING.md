...
| No executed line falls back to the locale encoding | `test_the_runtime_encoding_guard_is_enabled_in_ci` | PEP 597 raises `EncodingWarning` from inside CPython, so it sees shapes no static rule can name: `subprocess.run(text=True)` and `logging.RotatingFileHandler` are how it found its first two. It is **not** a replacement for the row above — it only fires on a line a test executes, so the two are kept together because they fail in opposite directions: the static rule is complete over the code and limited to two method names, this is complete over shapes and limited to what runs |
