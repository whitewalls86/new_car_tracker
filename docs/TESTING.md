...
| G21 | **46 of 100 routes return a status code no route declares, and not one route declares a single code today.** Every service's OpenAPI schema declares exactly `200` and `422` -- FastAPI's defaults -- while the suite asserts 303, 307, 308, 400, 401, 403, 404, 409, 422, 500 and 503 across **137 assertions**. Every real code is raised inside a handler body and surfaces nowhere a machine can read, so the schema is not a weak contract but a false one, and anything generated from it inherits the falsehood. Codes: 503 (15), 303 (13), 409 (11), 500 (10), 404 (7), 400 (5), 422 (3), 403, 307 and 308. First measured 2026-09-07 at 42 of 85; **re-measured 2026-09-08 against `d6e3a6d`**, and the difference is grain rather than drift -- six `ops` handlers are `api_route(..., methods=["GET", "HEAD"])` and serve two routes each. `dashboard` contributes nothing because it still cannot be imported ([G18](#the-gap-list)) | Declared must equal produced, both directions -- an undeclared code fails, and a declared code nothing raises fails too | Plan 162, Stage Y |
| G27 | **A handler does not observe the effects it causes. 13 functions execute a `WHERE`-bearing `UPDATE`/`DELETE` with no `RETURNING` and no `rowcount` read**, across `ops/routers/` (admin, users, scrape, snapshots, maintenance), `processing/` and `archiver/`, and a further 11 sites swallow an effect's exception and fall through to the success path. `toggle_search` is the worked example and it passes [G21](#the-gap-list)'s rule cleanly: it declares 303, produces 303, and returns that redirect whether the search key existed or not. **There is no member where checking is worse** -- `_record_last_used` touching a row that is gone is exactly when you want to know, and `_reap_stuck_processing` updates a row it just read by primary key, so a `rowcount` of 0 there is a live concurrent-modification race passing silently. `revoke_user` stacks both halves: no row matched and the database raising are both reported to an admin as a successful revocation. Measured 2026-09-08 | **Four clauses, all asserted**, each proved by a mutation rather than claimed: the statement's own `rowcount`, a swallowed write, a discarded outcome object, and an unread response status. The gap grew as it was closed -- the last two were found by asking why the first two had missed `deploy_start`, and both had been in the tree for months. Only the two crossing a service boundary carry ledgers, and every entry there is a call to a `dbt_runner` endpoint that no longer exists, owned by Stage AA. **No ledger on either**, following the precedent `check_sql_execution_coverage.py` set when its own waiver list was deleted rather than kept empty. A swallow covering a read or a parse is exempt: it degrades honestly, and `scraper/processors/scrape_detail.py:196` is the control case, binding `minio_write_error` into the response it returns | Plan 162, Stage Y |
| G28 | **A declared code that nothing exercises. 68 (route, code) pairs, 46 asserted by some test and at most 22 by none.** The ceiling is soft in the direction this document has been caught by before -- `tests/ops/routers/test_coordination.py:156` asserts a `parametrize`-injected code, which a literal scan reads as no assertion, the same blindness Stage H fixed for paths -- so the true figure is lower and this stage re-measures with `_parametrized_strings`. **This is not a new rule but the endpoint-coverage rule above, strengthened**: that rule says every route is reached *"and the test asserts the status code"*, singular, which every vacuous assertion satisfies. Only [G21](#the-gap-list)'s declarations make "every declared code" expressible, and it is the edit that answers whether a test could have failed -- not by grading an assertion, but by requiring the declared set to be covered. Measured 2026-09-08 | **Asserted** by `test_every_status_code_a_route_can_produce_is_asserted`, keyed on the codes a handler can **produce** rather than the ones it declares -- the two are equal by the rule above once the declarations land, and produced is the reading available before they do. Seeded at 11, drained by Stage Y's fourth step. Four handlers are declared un-attributable rather than skipped: `ops` serves `/deploy/start` and `/admin/deploy/start` from two handlers whose decorator strings are identical, and Stage AA owns the legacy pair whose removal drains all four | Plan 162, Stage Y |
