...
| G29 | **The harness that proves every other rule can fail is proved by nothing.** `scripts/verify_testing_contract_mutations.py` is what every *demonstrated by X failing* exit in Plan 162 rests on, and its own docstring records that it is **not a CI step** -- so its **59 anchors across 22 files** are literal strings that can stop matching at any time, and nobody finds out until somebody remembers to run it. Measured 2026-09-09: 61 rules are named in the *Asserted by* column above, **41 are proved by a mutation and 20 are not**. The argument is a measurement rather than a conviction -- of the six rules Plan 162 Stage Y landed, **every one shipped with a bug that made the repository look healthier than it was and not one of those bugs failed a test**: the observation rule passed on a tree with a `rowcount` check deleted, and the coverage rule took 38 of 89 handlers out of scope while its failure list still read four. Both were caught by writing the mutation and watching it not fail | **The mutations cannot be derived and that is the finding**: `mutmut` and `cosmic-ray` prove the suite is sensitive somewhere, not that *this* rule catches *the defect it was written for*, and the description is what makes that claim. Two things around them derive cleanly -- every rule in the *Asserted by* column owes a mutation, one table with a second obligation rather than a new registry; and every `_edit` anchor must still match its file exactly once, which is a string search per entry and no mutation run at all. Stage Y broke five waivers keyed on line numbers by editing the file above them; anchors have the same fragility and none of the protection | Plan 162, Stage AF |
