...
#### April pack-00001, and the stale image that wrote it

The second pack (17,460 members, 100% verified, 0 deleted, resume correct) came
out a near-twin of the first: 392 frames against 387, 8.12 MiB of raw per frame
against 8.12, and 3,856 B/member against 3,884 — a 0.7% difference where the
alignment fix projected ~38%.
...
```
docker exec cartracker-archiver python -c   "import shared.packfile as p; print(p.DEFAULT_FRAME_TARGET_BYTES//1024//1024, 'MiB')"
```
...
This is also what the label in the next section means: pack-00000 and pack-00001
are called *pre-fix* there because of this, not because they predate the commit.
