...
| 3 | [138](plans/plan_138_public_surface_refresh.md) | Public surface refresh | Stage 1f reconcile against the published writings — audit done 2026-08-31, copy pass built 2026-09-01 and in review (CAR-59). The surface scope question is answered: the published articles are out of scope as point-in-time artifacts, so this plan owns the README and the landing page only. That answer raised **Stage 1g** — the landing page links the corpus under a list of immutable facts only (title, date, URL), since a per-article annotation would rot every time the tree moves. Its weight is the add-an-article procedure: a two-way reconciliation against both surfaces, held by Stage 1c's commit gate. **Stage 1h** closes the last direction: 1c fires only when a surface is staged, so a plan that changes the system and edits no prose never reaches it — the `close-out` skill gains one cheap question that proposes and never writes | **Y** | -- | 84 | L | **Plan 143 completed 2026-08-20**, so the stats contract exists and Stage 4/PR D are unblocked. **Its project-updates snapshot now reads [completed_plans.md](planning/completed_plans.md)**, since Plan 146 removed this file's duplicate Completed table. **Stages 1e and 3d were added 2026-08-31** — publish `docs/recaps/` through the same build-time projection; per-service subdomains were scoped out to [Plan 165](plans/plan_165_service_subdomain_routing.md). Land before the next major platform milestone |
