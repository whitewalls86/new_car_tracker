...
|---:|---|---|---|---|---|---|---|---|
| 1 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stage 6b — mechanise the encoding-sensitive I/O guard, so a Windows-only defect CI cannot see fails before it reaches master | **Y** | -- | 75 | L | **Stages 0–6 complete; Stage 6 CAR-50, 2026-09-01.** `ROUTE_WAIVERS` is empty; 56 waivers remain, all Layer 2. **Stage 6b was added 2026-09-01** after a Windows-only encoding defect broke master — the failure G13's own exception predicted. Stage 7 is the largest remaining slice |
