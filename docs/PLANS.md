...
|---:|---|---|---|---|---|---|---|---|
| 1 | [161](plans/plan_161_testing_contract.md) | Testing contract | **The answers and the contract landed 2026-08-31 (CAR-33)** — `docs/TESTING.md`, and `ARCHITECTURE.md`'s section is now a pointer. Remaining: the `.claude/skills/` reviewer and the test that asserts the contract (CAR-34) | **Y** | -- | 87 | S | Blocks Plans 162 and 139. `ARCHITECTURE.md`'s Testing Strategy section becomes this plan's territory — rewritten as the contract or replaced by a pointer, never left as a second description |
| 2 | [142](plans/plan_142_planned_host_maintenance.md) **Stage 4** | Scoped operational coordination and host maintenance | Fix the four Stage 2/3 defects the 2026-08-29 scoping found — deferred-reboot kernel target, one-off manifest capture, the `oauth2-proxy` release-gate exemption, the client's API default — then deploy `ops` and `container-health` and run the window from the Stage 4 run sheet | **Y** | -- | 86 | M + first observed window | **Stage 3 is deployed** — `ops` 2026-08-30 (carried by Plan 147's deploy), `container-health` rebuilt deliberately the same evening; the gate answers. The four defects remain and the plan document holds them. **Both external blockers are now clear** — Plan 145 Stage 6 finished 2026-08-30, and Plan 158 fixed and proved the coordination gate deadlock that made declaring intent unsafe, so the window is workable as soon as the four defects are fixed |
| 3 | [139](plans/plan_139_test_suite_maintenance.md) **Stage F** | CI's database does not model production's schemas | Give `tests/integration/sql/` the Airflow schema so `ops` queries crossing into it can be executed at all | **Y** | -- | 78 | S | Ships ahead of Plans 161 and 162, and de-risks Plan 142 Stage 4's window: a bug of exactly this shape hung the first production deploy of the coordination gate. **Stage H (XS) is the only other stage left in Plan 139** — B, C, E and half of D move to Plan 162 |
| 4 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stub until Plan 161 lands — then the census, the 267s dbt-job split, dbt against the Plan 120 snapshot, and the suites on real Compose services | **N** | Plan 161 | 75 | XL | Absorbs Plan 139 Stages B, C, E and the markers half of D. **XL is a placeholder** Plan 161 replaces with a real estimate; do not start the census before the contract exists |
...
[**completed_plans.md**](planning/completed_plans.md) — 116 rows, newest first, one row
