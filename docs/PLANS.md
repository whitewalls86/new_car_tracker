...
|---:|---|---|---|---|---|---|---|---|
| 1 | [142](plans/plan_142_planned_host_maintenance.md) | Planned host maintenance and production quiescence | Stage 1's drain contract — a maintenance state record distinct from deploy intent, plus named active-work counts replacing the ambiguous `number_running`. Independent of Phase B: a pool gates, it does not count | **Y** | -- | 86 | M + first observed window | **Stage 0 items 1-5 merged 2026-08-24** (PR #231). Item 3 Phase A is deployed — the `maintenance` pool is live at 16 slots with all five pooled tasks confirmed running in it. Items 6-7 merged (PR #232) and pulled onto the VM 2026-08-25, but **not applied** — both are service config, so `caddy` still runs `restart=no` and the four Airflow services still hold the old JWT secret until Phase B's window recreates them. **Phase A's 24h soak closed clean 2026-08-25**, called early at ~21.5h: 258/258, 258/258 and 86/86 success-vs-failure identical to the prior day across the three pooled DAGs, 1,118 pooled task instances, zero slots held, zero stuck. **Phase B is now gated only on Plan 136's Stage 3a baseline read (~19:40 UTC 2026-08-25)**, since an hour-long hold puts a trough in the `trawl` memory curve 3b sizes against. Runbook §10 holds the end-to-end window. Stage 1 declares its own `maintenance` pool and Plan 136 Stage 3d a separate `solver` — a task has exactly one pool, so the shared-pool arrangement this row claimed before 2026-08-23 could not have worked. **Fully unblocked 2026-08-21** — its resume gate required soaked Plan 140 health coverage, and Plan 144 hardened the deploy script Stage 2 consumes |
