...
|---:|---|---|---|---|---|---|---|---|
| 1 | [136](plans/plan_136_solver_recycle_and_liveness.md) | Solver recycle and real liveness | Settle whether `BROWSER_RECYCLE_AFTER_CONTEXTS` actually recycles — it did not touch a 4-day-old browser — then re-derive Stage 3's interval against memory headroom rather than the 22-day uptime figure | **Y** | -- | 98 | M | Stage 0 verified; Stage 1 moved to Plan 143; 0b shipped as Plan 140 Stage 2. **Unblocked 2026-08-22 by D7** — the resume gate's "until the rate bends" half is met, and the involuntary OOM recycle Stage 3 was sized against has stopped firing. The `POST` verb on the socket proxy is separable, as one added verb on the existing `docker-socket-proxy` grant rather than a second socket path |
