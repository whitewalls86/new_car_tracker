...
|---:|---|---|---|---|---|---|---|---|
| 1 | [136](plans/plan_136_solver_recycle_and_liveness.md) | Solver recycle and real liveness | An **observation window, not code** — the 24h baseline is flat and healthy, so the recycle interval has nothing to be chosen from until the rate bends or `trawl` nears the 22-day uptime that preceded the incident | **N** | `trawl` decay window [~2026-09-09] | 98 | M | Stage 0 verified; Stage 1 moved to Plan 143; 0b shipped as Plan 140 Stage 2. **Not workable until ~2026-09-09.** The `POST` verb on the socket proxy is separable and *is* workable now, as one added verb on the existing `docker-socket-proxy` grant rather than a second socket path |
