...
| 2 | [134](plans/plan_134_archiver_endpoint_failure_contract.md) | Archiver endpoint failure contract | Stage C — enforcement, one endpoint per deploy at least 48h apart, in ascending blast radius: compact, then staging, then silver | **Y** | -- | 88 | S | Stage B's observation window closed clean 2026-09-07. Stage C flips one endpoint per deploy at least 48h apart, so each deploy is its own safe stopping point |
