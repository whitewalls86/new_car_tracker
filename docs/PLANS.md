...
| 4 | [147](plans/plan_147_scrape_state_ownership.md) **Stage 4** | Scrape state ownership — separating fetch from enrichment | `V049` contract: drop `last_detail_scraped_at` and the dual write, and collapse the Stage 1 `COALESCE` in both view predicates. Must repeat `V048`'s backfill in the same transaction first | **Y** | -- | 75 | S | Nothing. Found by Plan 142 Stage 0, which hit the loop while scoping a maintenance pause. **Supersedes Plan 115's mechanism while preserving its goal** — the guard moves next to the fetch so new causes cannot reopen the loop. Stages 1–3 are deployed and verified in production — 2,000 fetches, 0 repeats, `results_processing` paused 81 minutes on 2026-08-30 — and Plan 142's held set has dropped `scrape_detail_pages`. **Stage 4 is the only irreversible step**; its `V049` must repeat the `V048` backfill before collapsing the `COALESCE`, or 43 legacy-only rows re-enter the loop |
...
[**completed_plans.md**](planning/completed_plans.md) — 115 rows, newest first, one row
