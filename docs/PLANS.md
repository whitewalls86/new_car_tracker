...
|---:|---|---|---|---|---|---|---|---|
| 1 | [148](plans/plan_148_docs_only_ci_fast_path.md) | Docs-only CI fast path | Add a fail-closed changed-path classifier and docs-tests job, then skip the heavy jobs for docs-only diffs | **Y** | -- | 68 | XS | Independent CI-only change. Verify one docs-only PR reaches a mergeable state with heavy jobs skipped, and one mixed diff still selects full CI. |
