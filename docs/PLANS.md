...
|---:|---|---|---|---|---|---|---|---|
| 1 | [158](plans/plan_158_coordination_gate_deadlock.md) | Coordination gate deadlock | Stage 0 — record recognition and recovery in the host-maintenance runbook, since the deadlock is live in production until Stage 1 deploys | **Y** | -- | 90 | S | Nothing. Found 2026-08-30 deploying Plan 147 Stage 2. **Blocks every deploy and Plan 142's maintenance window** — the drain waits on `coordination_gate_observations`, which the gate sensor cannot write while deploy intent is pending. Stage 0 is documentation and carries no deploy, so it is a safe stopping point |
...
| 16 | [156](plans/plan_156_block_page_detection.md) | Block-page detection beyond Cloudflare | Extend the marker set in `shared/challenge.py` so the parser and the scraper's solver-outcome counter agree, keeping the `initial-activity-data` safety gate, and route an Akamai block to the `skip` path that leaves `blocked_cooldown` intact | **N** | Plan 147 | 42 | S | Plan 147 lands first; the marker-set extension and its tests are a safe stopping point before the routing change |
