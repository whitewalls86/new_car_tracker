...
|---:|---|---|---|---|---|---|---|---|
| 1 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stage T — shared fixtures: what the suite duplicates at 3,988 tests | **Y** | -- | 75 | L | Nothing blocks it. CAR-80 has no blockers for Stage T. Safe stopping point after Stage T. |
| 2 | [134](plans/plan_134_archiver_endpoint_failure_contract.md) | Archiver endpoint failure contract | Stage C deploy 2 of 3 — flip `/flush/staging/run`. Deploy 1 (`/compact/silver/run`) live 2026-09-07 19:25Z; its 48h gate opens 2026-09-09 19:25Z, deploy 3 on 09-11, final gate closes 09-13 19:25Z | **N** | Soak until 2026-09-09 19:25Z | 88 | S | Stage B's observation window closed clean 2026-09-07. Stage C flips one endpoint per deploy at least 48h apart, so each deploy is its own safe stopping point |
| 3 | [151](plans/plan_151_distributed_tracing_and_runtime_topology_audit.md) | Distributed tracing and runtime topology audit | Stage A — define the bounded instrumentation experiment, telemetry contract, resource budgets, and Plan 142 declared-graph export | **Y** | -- | 72 | M + 7d observation | Plan 142 remains authoritative safety policy; begin metrics-only, and add Tempo only if aggregate telemetry cannot answer a recorded diagnostic question |
| 4 | [170](plans/plan_170_container_image_reclaim_policy.md) | Container image reclaim policy | Stage B — make the deploy prune the build cache it just produced: `docker builder prune --keep-storage 4GB -f`, after health verification, in `scripts/deploy.sh` and `scripts/redeploy.sh`, non-fatal. **Stage A decided both rules on 2026-09-08** from a read-only production pass: build cache grew 2.04 → 7.52 GB in 8 days while images stayed flat at ~21 GB, so it is ~90% of growth and is produced only by those two build paths — the reclaim belongs to the producer, not a clock. Images get no automated retention: `docker image ls -a -f dangling=true` returns zero, so the rollback window is already zero and no retention rule can raise it. No scheduled job gets built. Stage C is the statically derived keep-set with its runbook block asserted by test, plus a one-time ~2.8 GB image sweep | **Y** | -- | 82 | S | Consumes Plan 142's `maintenance-running-set.txt` classes read-only — `aux-paused` and `on-demand` are what separate an image paused by decision from garbage, and `docker image prune -a` cannot read them. Plan 142 is in closeout and owes no code, so the manifest is input, never a dependency to change. Sequenced ahead of Plan 125 because Gates C/D productionize `cartracker-lakehouse`, the fleet's largest image, which grows to ~2.34 GB on its next rebuild. That stopping point resolved on 2026-09-08 in a variant of its own escape hatch: maintenance windows do not flatten the growth, but the producer does, so the plan shrank to two deploy-script lines and a tested keep-set rather than a scheduled job. The manifest stays a read-only input — and Compose already stamps `com.docker.compose.project`/`service` on all 12 images this repo builds, keyed the same way the manifest is, so the keep-set join needs no Docker at runtime |
| 5 | [173](plans/plan_173_machine_credential_lifecycle.md) | Machine credential lifecycle | Stage A — table-backed machine tokens behind the `_resolve_token` seam, then retire the environment-variable fallback | **Y** | -- | 82 | XS | Lands against the seam added alongside Plan 162 Stage P; safe to stop after the first deploy, with the env fallback still live |
...
| 9 | [152](plans/plan_152_scheduled_worker_lifecycle.md) | Scheduled worker lifecycle and one-shot execution | Stage 0 — classify every Compose service by lifecycle, then select the narrow launch authority and its refusal boundary | **Y** | -- | 64 | S | **Trigger fired** — Plan 142 Stage 1 is live. A one-shot that outlived its work was counted as admitted work for 14 hours on 2026-08-29 and was the first blocker on the deploy hang. Plan 159 is triggered by this landing |
| 10 | [112](plans/plan_112_refresh_policy_backtesting.md) | Adaptive-refresh backtesting | Resume policy backtest/model gates on pinned Iceberg snapshots | **N** | Plan 125 Gate D | 76 | L | Plan 125 stable Iceberg-native inputs |
| 11 | [113](plans/plan_113_production_adaptive_refresh.md) | Production adaptive refresh | Promote one reviewed, pinned policy into ops claim logic | **N** | Plan 112 | 74 | M | Approved Plan 112 result; no live model dependency |
...
| 16 | [156](plans/plan_156_block_page_detection.md) | Block-page detection beyond Cloudflare | Extend the marker set in `shared/challenge.py` so the parser and the scraper's solver-outcome counter agree, keeping the `initial-activity-data` safety gate, and route an Akamai block to the `skip` path that leaves `blocked_cooldown` intact | **Y** | -- | 42 | S | Plan 147 landed 2026-08-30; the marker-set extension and its tests are a safe stopping point before the routing change |
...
[**completed_plans.md**](planning/completed_plans.md) — 122 rows, newest first, one row
