...
|---:|---|---|---|---|---|---|---|---|
| 1 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stage 8 (CAR-52) — the Layer 2 suite that asserts nothing: `tests/integration/sql/test_dashboard_queries.py` is 25 tests and 0 assertions, so a renamed mart column passes green and `KeyError`s in production | **Y** | -- | 75 | L | **Stages 0–7 complete, and 6c with them.** `ROUTE_WAIVERS`, `ENCODING_WAIVERS` and `LAYER_2_WAIVERS` are all empty; every waiver that remains was opened by Stage 7 itself |
