...
|---:|---|---|---|---|---|---|---|---|
| 1 | [160](plans/plan_160_promtail_contract_checker_reliability.md) | Promtail contract checker reliability | Stage 0 — reproduce the shutdown race locally under load, then hold stdin open, wait for the corpus's expected entry count, and add the inconclusive verdict | **Y** | -- | 85 | XS | Split from Plan 139 Stage G. Four occurrences, two of them the same branch disagreeing with itself; the checker cannot distinguish a lost entry from a policy drop, so its verdict is untrustworthy in both directions. Stage 0 is mandatory — a race that has not been observed failing cannot be verified fixed |
