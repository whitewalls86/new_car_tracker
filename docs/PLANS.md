...
| 6 | [134](plans/plan_134_archiver_endpoint_failure_contract.md) | Archiver endpoint failure contract | Stage C's exit — deploy 3's 48h gate, and a production page from `hourly_analytics_refresh` naming a failed task and quoting its `failure_reason`. Deploy 1 (`/compact/silver/run`) live 2026-09-07 19:25Z and deploy 2 (`/flush/staging/run`) live 2026-09-10 19:16Z, both gates clean; deploy 3 (`/flush/silver/run`) lands with this row | **N** | Soak until 2026-09-14 | 88 | S | Stage B's observation window closed clean 2026-09-07. A clean soak does not supply the page — that takes a real failure or a deliberate one. Stage D follows once Stage C has held a week |
