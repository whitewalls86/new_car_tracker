...
| 3 | [154](plans/plan_154_container_log_coverage.md) **Stage A** | Container log coverage | Stage A — classify all 28 expected-running services and measure candidate log volume before admission | **Y** | -- | 70 | S + 7d observation | Plan 141 is complete; stop after Stage A unless measurements justify admitting new streams |
| 4 | [151](plans/plan_151_distributed_tracing_and_runtime_topology_audit.md) | Distributed tracing and runtime topology audit | Stage A — define the bounded instrumentation experiment, telemetry contract, resource budgets, and Plan 142 declared-graph export | **Y** | -- | 72 | M + 7d observation | Plan 142 remains authoritative safety policy; begin metrics-only, and add Tempo only if aggregate telemetry cannot answer a recorded diagnostic question |
| 5 | [170](plans/plan_170_container_image_reclaim_policy.md) | Container image reclaim policy | Stage A — decide two retention rules, one per pool. **The trend question is answered:** the ~9 GB between the runbook's 2026-08-29 reading (72%, 29 GB) and production on 2026-09-01 (53%, 20 GB) was a manual `docker builder prune`, confirmed by the maintainer and corroborated in shell history, which records no `image prune` at all. Growth is monotonic in both pools and nothing scheduled reclaims either, so the escape hatch that would have folded this into Plan 142's procedure is closed. Build cache moved *into* scope on the same evidence — it regrew ~0 → 2.04 GB in about two days, faster than images accumulate. Its threshold needs a week of curve before it is fixed | **Y** | -- | 82 | S | Consumes Plan 142's `maintenance-running-set.txt` classes read-only — `aux-paused` and `on-demand` are what separate an image paused by decision from garbage, and `docker image prune -a` cannot read them. Plan 142 is in closeout and owes no code, so the manifest is input, never a dependency to change. Sequenced ahead of Plan 125 because Gates C/D productionize `cartracker-lakehouse`, the fleet's largest image, which grows to ~2.34 GB on its next rebuild. Safe stopping point is after Stage A — if the trend is a sawtooth that maintenance windows already flatten, the outcome is a step in Plan 142's procedure and the plan stops there |
| 6 | [173](plans/plan_173_machine_credential_lifecycle.md) | Machine credential lifecycle | Stage A — table-backed machine tokens behind the `_resolve_token` seam, then retire the environment-variable fallback | **Y** | -- | 82 | XS | Lands against the seam added alongside Plan 162 Stage P; safe to stop after the first deploy, with the env fallback still live |
