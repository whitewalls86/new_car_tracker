...
|---:|---|---|---|---|---|---|---|---|
| 1 | [146](plans/plan_146_planning_system.md) | The planning system | Stages 0-4 are done; the invariant is now `tests/test_planning_docs.py`, and Stages 5-6 are the skills that automate the edits it constrains | **Y** | -- | 92 | M | **Maintainer's #1 priority, set 2026-08-21.** Stage 0 found considerably more than the five known disputes: 24 plan documents in no table, and 9 plans with no document *and* no archive row — including Plan 65, the auth stack, which shipped and vanished from the record for four months. Stage 5's skill is deliberately late; automating a structure still under argument encodes the argument |
