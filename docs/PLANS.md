...
|---|---|---|
| [164](plans/plan_164_cycle_close_ritual.md) | **2026-09-09** | Cycle 2's close, run end to end in the written order: `cycle-measures` fills Plan 149's Cycle 2 column from the closed cycle with any partial marked partial, `roll-cycle` reconciles the rollover against what Linear already moved, and `docs/recaps/2026-09-06.md` exists before any ref is deleted |
...
| [154](plans/plan_154_container_log_coverage.md) | **2026-09-15** | Stage C's seven-day window, opened by Stage B's deploy on 2026-09-08 03:13 UTC. Re-measure `trawl` and `postgres` lines/day and bytes/day against Stage A's baselines, recompute the 90-day Loki footprint against Plan 135's bounds, confirm each drop counter is doing the expected work — read from Promtail's own `/metrics`, not Prometheus — and record a keep/narrow/remove per service. Stage C's exit also asks for one representative error per admitted service reaching the error view, which `trawl` cannot produce: every line is INFO by Stage B's design. That clause needs a decision, not a reading |
...
| [170](plans/plan_170_container_image_reclaim_policy.md) | **2026-09-22** | `cartracker_path_bytes{path="/var/lib/containerd"}`, sampled daily at ~07:24 UTC, shows the store stepping down on deploys rather than ramping to a manual `docker builder prune`. The 14 days before the change were two monotonic ramps (23,329 → 29,577 MiB, then 19,795 → 24,924) split by one person-driven 10 GB cliff on 08-31. The 09-08 sample predates both Stage B's deploys and Stage C's sweep, so the series holds no post-change point. Row leaves when the window covers several deploys and shows a sawtooth with no cliff a person had to cause |
...
|---:|---|---|---|---|---|---|---|---|
| 1 | [183](plans/plan_183_minio_image_self_hosted.md) | The MinIO image, held where we control it | Stage A — ghcr holds production's exact MinIO build | **Y** | -- | 95 | S | Nothing blocks it. Heavy CI is red until Stage B lands, so it precedes every Plan 180–182 merge. |
| 2 | [180](plans/plan_180_seam_program.md) | The seam program | Stage A — the mechanisms exist | **Y** | -- | 85 | XL | Supersedes Plan 162 Stages AL(tail)/AM/AH/AC/AE; AD and AJ passed on to Plan 182 as its Stages G and H on 2026-09-11. 162's native cleanup runs in parallel, not as a gate. Safe stopping point after Stage B — the ideal rules and seeded ledgers are useful without any drain. |
| 3 | [185](plans/plan_185_one_python_base_image.md) | One Python base image | Stage A — a rule that every Python base names CI's Python and a Debian release | **Y** | -- | 80 | M | Nothing blocks Stage A. Lands before or during Plan 180 Stage K's ledger drain, so K copies two base images, not five. |
| 4 | [181](plans/plan_181_rule_readers_are_code.md) | The rule readers are code | Stage A — a test module defines tests, and the rest is a package | **Y** | -- | 85 | XL | Nothing blocks Stage A. Wants to land before Plan 180 Stage B, which writes a border guard per seam — each one a new reader, and the directory those are written into is decided by whether Stage A exists. Safe stopping point after Stage A: the package, its coverage entry and the boundary rule stop new unmeasured code being added, with nothing yet moved. |
| 5 | [182](plans/plan_182_what_a_unit_owes_itself.md) | What a unit owes itself | Stage A — the within-party frame written into the contract | **Y** | -- | 85 | XL | Nothing blocks Stage A. Stage B wants Plan 181 Stage A's package for its reader; Stage F wants Plan 180 Stage A's double builder. Absorbs Plan 180 Stages J and L as its G and H. Safe stopping point after Stage D: the frame, both ledgers and the mutation harness stand with one module converted |
| 6 | [134](plans/plan_134_archiver_endpoint_failure_contract.md) | Archiver endpoint failure contract | Stage C deploy 3 of 3 — flip `/flush/silver/run`, last deliberately: a 500 there skips a dbt build that would have been building on stale silver. Deploy 1 (`/compact/silver/run`) live 2026-09-07 19:25Z, its 48h gate closed clean 2026-09-09 19:25Z; deploy 2 (`/flush/staging/run`) live 2026-09-10 19:16Z | **N** | Soak until 2026-09-12 19:16Z | 88 | S | Stage B's observation window closed clean 2026-09-07. Stage C flips one endpoint per deploy at least 48h apart, so each deploy is its own safe stopping point |
| 7 | [184](plans/plan_184_replace_minio_with_garage.md) | Replace MinIO with Garage | Stage A — Garage proven against our S3 surface | **N** | Plans 180–182 | 82 | XL | Waits behind Plans 180–182 by decision; must land before Plan 125's production Iceberg writes; Garage is vendored through Plan 183. |
| 8 | [125](plans/plan_125_duckdb_to_iceberg_migration.md) | DuckDB-to-Iceberg analytics migration | Gate C production runtime measurement, then Gate D reader inventory/dual-run | **N** | Plan 184 | 81 | XL | Plan 120 closeout; swap Plan 143's producer adapter while preserving its snapshot and metric contracts. **Start from `feature/plan-125-portability-audit`, not `master`** — 17 unmerged commits (last 2026-07-21) carry the replay and scale harnesses and the local reproduction of the VM OOM |
| 9 | [177](plans/plan_177_service_log_emission.md) **Stage A** | Service log emission | Stage A — turn down `docker-socket-proxy`'s access logging, the cheapest and least risky of the four | **Y** | -- | 80 | M | Split out of Plan 154 Stage A. Each stage is one service and its own safe stopping point; Stage D is a decision with no deploy and Stage E is the only edge change |
| 10 | [168](plans/plan_168_generated_knowledge_substrate.md) | Generated knowledge substrate | Stage 0 — the exposure vocabulary, declared in each model's `schema.yml` `meta`, before any generator exists | **Y** | -- | 70 | M | Nothing blocks it — the dbt manifest exists today. Must precede Plan 150 Stage 0a, which is where its priority comes from. Safe stopping point is after Stage 2: a generated catalog with a CI staleness gate is independently useful |
| 11 | [179](plans/plan_179_derived_service_call_graph.md) | Derived service call graph | Stage A — the deriver over Compose env and source literals, its checked-in snapshot, and `--check` | **Y** | -- | 68 | XS | Nothing blocks it. Raised by Plan 151 Stage A, which declined the runtime telemetry pipeline after static derivation produced 62 undeclared call edges for ~80 lines. Consumes `ops/coordination_contract.py` read-only, so Plan 142's closeout is undisturbed. Safe stopping point after Stage A — a checked-in snapshot with `--check` is useful without the extra parsers or the CI gate |
| 12 | [152](plans/plan_152_scheduled_worker_lifecycle.md) | Scheduled worker lifecycle and one-shot execution | Stage 0 — classify every Compose service by lifecycle, then select the narrow launch authority and its refusal boundary | **Y** | -- | 64 | S | **Trigger fired** — Plan 142 Stage 1 is live. A one-shot that outlived its work was counted as admitted work for 14 hours on 2026-08-29 and was the first blocker on the deploy hang. Plan 159 is triggered by this landing |
| 13 | [112](plans/plan_112_refresh_policy_backtesting.md) | Adaptive-refresh backtesting | Resume policy backtest/model gates on pinned Iceberg snapshots | **N** | Plan 125 Gate D | 76 | L | Plan 125 stable Iceberg-native inputs |
| 14 | [113](plans/plan_113_production_adaptive_refresh.md) | Production adaptive refresh | Promote one reviewed, pinned policy into ops claim logic | **N** | Plan 112 | 74 | M | Approved Plan 112 result; no live model dependency |
| 15 | [155](plans/plan_155_log_dashboards.md) **Stage 0** | Log dashboards and aggregate triage | Stage 0 — define the bounded dashboard questions against Plan 141's landed labels and fields | **Y** | -- | 62 | S | Plan 141 is complete. Plan 154 supplies additional streams but does not block Stage 0; keep behind Plan 162 so it does not delay the next feature milestone |
| 16 | [178](plans/plan_178_role_grant_scoping.md) | Role grant scoping | Stage A — the measured needs matrix, per role, from the `.sql` corpus | **Y** | -- | 60 | M | Nothing blocks it — Stage A reads the `.sql` corpus Plan 162 Stage L already produced, read-only. Safe stopping point is after Stage D: the needs matrix is measured, Layer 2 fails on a missing privilege and the assertion runs, with no grant yet changed |
| 17 | [119](plans/plan_119_lakehouse_governance.md) | Lakehouse governance | Add measured catalog controls and auditability | **N** | Plan 125 | 58 | L | Stable Plan 125 catalog and reader contracts |
| 18 | [126](plans/plan_126_basic_event_streaming.md) | Basic event streaming | Prove transport, replay, and one low-risk consumer | **N** | Plans 125, 112, 113 | 49 | XL | Plans 125 and 112/113 clarify stable event semantics |
| 19 | [156](plans/plan_156_block_page_detection.md) | Block-page detection beyond Cloudflare | Extend the marker set in `shared/challenge.py` so the parser and the scraper's solver-outcome counter agree, keeping the `initial-activity-data` safety gate, and route an Akamai block to the `skip` path that leaves `blocked_cooldown` intact | **Y** | -- | 42 | S | Plan 147 landed 2026-08-30; the marker-set extension and its tests are a safe stopping point before the routing change |
| 20 | [127](plans/plan_127_streaming_adaptive_scrape_control.md) | Streaming adaptive scrape control | Add closed-loop control behind batch-parity and rollback gates | **N** | Plan 126 | 42 | XL | Plan 126 plus approved Plan 112/113 behavior |
| 21 | [69](plans/plan_69_terraform.md) | Terraform IaC | `terraform import` the existing VM/network/firewall until `plan` shows no diff against production | **Y** | -- | 66 | M | **Moved out of the backlog 2026-08-20** — its trigger was "a second environment is approved", and Plan 121 is that environment. Must land before Plan 121, not after |
| 22 | [121](plans/plan_121_staging_environment.md) | Staging environment | Stand up the smallest fixture-backed deployed environment, provisioned from Plan 69's modules | **N** | Plan 69 | 63 | L | Plan 69 first, so staging and prod come from one module set instead of two hand-built hosts. Prefer after Plan 125 reader shape settles unless needed earlier for risky rollout |
| 23 | [186](plans/plan_186_image_refresh_cadence.md) | Owned images, refreshed on a cadence | Stage A — every owned image's source tag and tier recorded, and a rule that each has one | **N** | Plan 180 Stage K | 60 | L | Stage E also waits on Plan 121's staging. Safe stopping point after Stage C: owned images get monthly refresh PRs before the stored-data tier is rehearsed. |
| 24 | [150](plans/plan_150_analytics_product_and_bi_serving_layer.md) | Analytics product and BI serving layer | Stage 0 research — define candidate analytical products and audiences (0b), then compare serving (0f) and BI (0g) options and recommend a bounded architecture | **Y** | -- | 68 | XL, research-gated | Stage 0 ends with a written tool and architecture decision; Plan 150 remains behind Plans 126, 127, 69 and 121, and no warehouse or BI commitment is made before that decision. **Stage 0a is generated by Plan 168 Stage 3 — do not hand-write the model inventory** |
| 25 | [174](plans/plan_174_public_site_shape.md) | Public site shape | Stage A — settle the site's shape: how many reading surfaces, what `/` holds to reach them, and whether the planning documents join as a third corpus | **Y** | -- | 58 | L | Carved out of [Plan 138](plans/plan_138_public_surface_refresh.md) at its closeout on 2026-09-04, inheriting its Stages 1g, 3d and 3a plus the navigation pane it deferred behind Stage 8's destination inventory. Plan 138's last position on presentation shape is recorded in the plan document as input to argue with rather than as a constraint. Nothing blocks it — every piece is buildable against the tree today; it sits here on priority, not dependency. Safe stopping point after any stage; after Stage A the shape decision is durable on its own even if nothing is built on it |
| 26 | [163](plans/plan_163_documented_code_quality_fixes.md) | Documented code quality fixes | A register, not a project — pull an item only when already in that file for another reason | **Y** | -- | 5 | XS each | **Deliberately last, and the lowest priority in the index; it must never displace scheduled work.** Inherits the live remainder of Plans 73 and 106, and records what they already delivered so it is not redone |
...
| [171](plans/plan_171_coordination_failure_vocabulary.md) | Coordination failure vocabulary for the deploy and maintenance path | 60 | S | **Plan 134 Stage C lands** — its `failure_reason` shape is the candidate vocabulary here, and drafting a second one before it settles is the defect this plan is about |
...
| [169](plans/plan_169_public_analytics_agent.md) | Public analytics agent | 46 | L | **Plan 168 Stage 3 lands** — for Stages 1 and later, which read its generated public projection. Stage 0 is the threat model, budget and go/no-go; it is decision-only, needs nothing that does not exist today, and can run at any time. A public LLM endpoint on `/info` answering questions about the platform for recruiters and hiring managers; querying data is Stage 4, gated behind Stage 2 shipping and behind a bounded public view set the agent's credential is scoped to |
...
| [176](plans/plan_176_role_connection_limits.md) | Postgres role connection limits | 44 | XS | `up{job="postgres"}` gaps a second time, which would prove the cap is actively binding rather than latent. Plan 154 Stage B, if it admits Postgres to Loki, makes the `too many connections` FATAL visible the moment it recurs — so that admission is what turns this from a defect nobody would notice into one that reports itself |
...
[**completed_plans.md**](planning/completed_plans.md) — 125 rows, newest first, one row
