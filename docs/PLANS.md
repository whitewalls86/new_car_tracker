...
|---:|---|---|---|---|---|---|---|---|
| 1 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stage 7 — SQL execution from both directions: the `.sql` files no Layer 2 test reaches, and the module-level statements at `execute()` call sites the rule cannot see | **Y** | -- | 75 | L | **Stages 0–6b complete.** `ROUTE_WAIVERS` and `ENCODING_WAIVERS` are empty; 56 waivers remain, all Layer 2 — which is exactly Stage 7's target |
...
| 3 | [138](plans/plan_138_public_surface_refresh.md) | Public surface refresh | Stage 1h (CAR-61) — add the closeout question: did the landed work change a mechanism, name or quantity either surface states. It is the only pullable Stage 1 slice, and it closes the last of three directions: 1c's gate fires only when a surface is *staged*, so a plan that changes the system and edits no prose never reaches it, which is the class every Gate 0 defect came from. **Stage 1g is not executable** — it links the published corpus from the landing page under a list of immutable facts only, and is blocked on 3d's two open decisions (the stylesheet question, and whether `/recaps` leads with the newest week in full), both of which govern its markup. Stage 1f landed 2026-09-01 (CAR-59): four corpus framings drawn onto both surfaces with every fact verified against the tree, and the surface scope question answered — the published articles are out of scope as point-in-time artifacts, so this plan owns the README and the landing page only. Stage 2 discoverability remains the next substantive block | **Y** | -- | 84 | L | **Plan 143 completed 2026-08-20**, so the stats contract exists and Stage 4/PR D are unblocked. **Its project-updates snapshot now reads [completed_plans.md](planning/completed_plans.md)**, since Plan 146 removed this file's duplicate Completed table. **Stages 1e and 3d were added 2026-08-31** — publish `docs/recaps/` through the same build-time projection; per-service subdomains were scoped out to [Plan 165](plans/plan_165_service_subdomain_routing.md). Land before the next major platform milestone |
