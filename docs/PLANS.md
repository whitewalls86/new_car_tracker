...
|---:|---|---|---|---|---|---|---|---|
| 1 | [136](plans/plan_136_solver_recycle_and_liveness.md) | Solver recycle and real liveness | Stage 3a — publish per-container memory from the existing `container-health` exporter, the series every later slice reads and the one thing D7's "size against memory headroom" needs | **Y** | -- | 98 | M | Stage 0 verified; Stage 1 moved to Plan 143; 0b shipped as Plan 140 Stage 2. **Unblocked 2026-08-22 by D7** — the resume gate's "until the rate bends" half is met, and the involuntary OOM recycle Stage 3 was sized against has stopped firing. **D8 (2026-08-23) rewrote Stage 3 into four slices** — `BROWSER_RECYCLE_AFTER_CONTEXTS` counts Tier 3/4 contexts only and is blind to the leaking persistent context, and the `POST` verb is **not** separable as one added verb: `ALLOW_RESTARTS` narrows nothing once `CONTAINERS=1` is set, so it needs a second, strictly narrower proxy instance rather than a wider grant on the existing one |
