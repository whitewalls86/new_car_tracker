...
|---:|---|---|---|---|---|---|---|---|
| 1 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stage 9 (unticketed) — `airflow/dags` has no `.sql` convention and cannot reach one: no module under it imports `shared`, so `shared.query_loader` is unavailable and the DAG tree is the only place where 'production SQL is a `.sql` file' is structurally impossible | **Y** | -- | 75 | L | **Stages 0–7 complete, and 6c with them.** `ROUTE_WAIVERS`, `ENCODING_WAIVERS` and `LAYER_2_WAIVERS` are all empty; every waiver that remains was opened by Stage 7 itself |
...
| 3 | [172](plans/plan_172_plan_authoring_skill.md) | The plan-authoring skill | Stage 0 — name the two sections a plan owes the public, and the rule that keeps the first stable: it is edited when the plan's purpose changes, never because a stage landed | **Y** | -- | 80 | S | **Blocks [Plan 138](plans/plan_138_public_surface_refresh.md) Stage 9**, which is why it sits above a higher-scored row: Stage 9's code is safe to land first, but cannot be *finished* until the published window carries the section this plan creates, or its §4 rule contradicts its own surface. Safe stopping point after any stage. Raised 2026-09-02 from a measurement — 75% of `PLANS.md` commits changed published copy over 60 days, 35 of 59 being a slice rewrite with the same four plans in the window |
| 4 | [138](plans/plan_138_public_surface_refresh.md) | Public surface refresh | Stage 8 (CAR-67) — extract `docs/PUBLIC_SURFACE.md`: the durable public-surface rules leave this plan, and the destination inventory records what a reader is meant to reach | **Y** | -- | 84 | L | **Plan 143 completed 2026-08-20**, so the stats contract exists and Stage 4/PR D are unblocked. **Its project-updates snapshot now reads [completed_plans.md](planning/completed_plans.md)**, since Plan 146 removed this file's duplicate Completed table. **Stages 1e and 3d were added 2026-08-31** — publish `docs/recaps/` through the same build-time projection; per-service subdomains were scoped out to [Plan 165](plans/plan_165_service_subdomain_routing.md). Land before the next major platform milestone |
| 5 | [164](plans/plan_164_cycle_close_ritual.md) | Cycle-close ritual | Stage 0 — write the close order down, then Stage 3's git-ref and worktree hygiene; both are independent of Linear and buildable now | **Y** | -- | 70 | S | Stages 1 and 2 need a *closed* cycle to read and so cannot run before 2026-09-07; Stage 3 is independently shippable and survives Plan 149's keep/change/remove decision either way. Safe stopping point is after any stage — the package is ordered, not atomic |
| 6 | [154](plans/plan_154_container_log_coverage.md) **Stage 0** | Container log coverage | Stage 0 — classify all 28 expected-running services and measure candidate log volume before admission | **Y** | -- | 70 | S + 7d observation | Plan 141 is complete; stop after Stage 0 unless measurements justify admitting new streams |
