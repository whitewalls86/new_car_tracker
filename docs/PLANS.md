...
Plan 136 has no executable step until September.
...
|---:|---|---|---|---|---|---|---|---|
| 1 | [146](plan_146_planning_system.md) | The planning system | Stage 0 -- reconcile the plans whose status three tables disagree on, then backfill from git, collapse the tables, and give `docs/` a hierarchy | **Y** | -- | 92 | M | **Maintainer's #1 priority, set 2026-08-21.** Found by the Plan 144 closeout the same day. `PLANS.md` has four surfaces asserting plan status and no rule for which wins: Plan 135 is declared complete in prose and recorded in neither completion record, Plan 114 is completed *and* blocked, and the Plan inventory covers 30 of 72 plan files. `docs/` is 98 files and zero directories, so Stage 3 gives it one. Stage 5 is a `plans` skill, deliberately last -- automating a structure still under argument encodes the argument |
