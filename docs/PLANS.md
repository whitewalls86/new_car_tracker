...
|---:|---|---|---|---|---|---|---|---|
| 1 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stage 6c — every service contract produces an intent row the database accepts: dashboard and pgadmin map to no surfaces, so `/deploy/start` writes `scope='[]'` against a CHECK that forbids it | **Y** | -- | 75 | L | **Stages 0–6b complete.** `ROUTE_WAIVERS` and `ENCODING_WAIVERS` are empty; 56 waivers remain, all Layer 2 — which is exactly Stage 7's target |
