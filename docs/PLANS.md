...
|---:|---|---|---|---|---|---|---|---|
| 1 | [162](plans/plan_162_testing_census_and_restructure.md) | Testing census and CI restructure | Stage U — every skip in CI is declared, or the run fails | **Y** | -- | 75 | L | **Stages A–P complete** (legacy 0–10). Every waiver ledger the plan opened is still shrinking; Stage N replaced Stage L's two SQL rules with one keyed on the statement rather than its container |
...
| 3 | [175](plans/plan_175_commit_gate_bundling.md) | Commit gate bundling hole | Stage A — enforcement moves inside `git commit`: a tracked `.githooks/pre-commit` runs the same stamp check, and the `PreToolUse` hook refuses when the install is missing | **Y** | -- | 90 | XS | Nothing blocks it — the gate it repairs is in the tree today, and every exit verifies locally in a disposable clone with no deploy. One stage, so there is no intermediate stopping point: until it lands, a bundled `git add … && git commit` or a `git commit -am` passes the gate while it reads an empty index |
...
| [171](plans/plan_171_coordination_failure_vocabulary.md) | Coordination failure vocabulary for the deploy and maintenance path | 60 | S | **Plan 134 Stage 2 lands** — its `failure_reason` shape is the candidate vocabulary here, and drafting a second one before it settles is the defect this plan is about |
...
[**completed_plans.md**](planning/completed_plans.md) — 121 rows, newest first, one row
