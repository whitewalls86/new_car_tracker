...
|---:|---|---|---|---|---|---|---|---|
| 1 | [149](plans/plan_149_linear_execution_layer.md) | Linear as the weekly execution layer | Stage 0 bootstrap and seed no more than eight issues | **Y** | -- | 72 | XS + three-cycle observation | One-day bootstrap, then move to closeout while the three-cycle experiment runs; repository docs remain authoritative |
| 2 | [142](plans/plan_142_planned_host_maintenance.md) | Planned host maintenance and production quiescence | Stage 1's drain contract — a maintenance state record distinct from deploy intent, plus named active-work counts replacing the ambiguous `number_running`. Independent of Phase B: a pool gates, it does not count | **Y** | -- | 86 | M + first observed window | **Stage 0 items 1-5 merged 2026-08-24** (PR #231). Item 3 Phase A is deployed — the `maintenance` pool is live at 16 slots with all five pooled tasks confirmed running in it. Items 6-7 built, awaiting merge. **Phase B is gated on 2026-08-25**: Phase A's 24h soak completes 03:47 UTC, and Plan 136's Stage 3a baseline must be read first, since an hour-long hold puts a trough in the `trawl` memory curve 3b sizes against. Runbook §10 holds the end-to-end window. Stage 1 declares its own `maintenance` pool and Plan 136 Stage 3d a separate `solver` — a task has exactly one pool, so the shared-pool arrangement this row claimed before 2026-08-23 could not have worked. **Fully unblocked 2026-08-21** — its resume gate required soaked Plan 140 health coverage, and Plan 144 hardened the deploy script Stage 2 consumes |
...
| 14 | [121](plans/plan_121_staging_environment.md) | Staging environment | Stand up the smallest fixture-backed deployed environment, provisioned from Plan 69's modules | **N** | Plan 69 | 63 | L | Plan 69 first, so staging and prod come from one module set instead of two hand-built hosts. Prefer after Plan 125 reader shape settles unless needed earlier for risky rollout |
| 15 | [139](plans/plan_139_test_suite_maintenance.md) **Stage C** | Profile the 92s `tests/integration/dbt/` step | Run it with `--durations=20` in CI and record the per-test breakdown before proposing any change | **Y** | -- | 60 | S | Measurement only; CI-only work — do not pip-install dbt locally |
...
[**completed_plans.md**](planning/completed_plans.md) — 110 rows, newest first, one row
