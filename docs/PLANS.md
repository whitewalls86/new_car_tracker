...
| 2 | [134](plans/plan_134_archiver_endpoint_failure_contract.md) | Archiver endpoint failure contract | Stage C deploy 3 of 3 — flip `/flush/silver/run`, last deliberately: a 500 there skips a dbt build that would have been building on stale silver. Deploy 1 (`/compact/silver/run`) live 2026-09-07 19:25Z, its 48h gate closed clean 2026-09-09 19:25Z; deploy 2 (`/flush/staging/run`) lands with this row | **N** | Soak until 2026-09-12 | 88 | S | Stage B's observation window closed clean 2026-09-07. Stage C flips one endpoint per deploy at least 48h apart, so each deploy is its own safe stopping point |
