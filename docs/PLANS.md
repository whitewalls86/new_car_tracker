...
| [164](plans/plan_164_cycle_close_ritual.md) | **2026-09-14** | Cycle 4's seed, made by `fill-cycle` on 2026-09-14, carries both marks on every issue it creates and on nothing else — the `seeded` label and `Seeded 2026-09-14 into Cycle 4.` — and a provisional `cycle-measures` read that day counts exactly that set as seeded. Row leaves, and the plan archives, when both hold |
...
|---:|---|---|---|---|---|---|---|---|
| 1 | [180](plans/plan_180_seam_program.md) | The seam program | Stage A — the mechanisms exist | **Y** | -- | 85 | XL | Supersedes Plan 162 Stages AL(tail)/AM/AH/AC/AE; AD and AJ passed on to Plan 182 as its Stages G and H on 2026-09-11. 162's native cleanup runs in parallel, not as a gate. Safe stopping point after Stage B — the ideal rules and seeded ledgers are useful without any drain. |
| 2 | [185](plans/plan_185_one_python_base_image.md) | One Python base image | Stage A — a rule that every Python base names CI's Python and a Debian release | **Y** | -- | 80 | M | Nothing blocks Stage A. Lands before or during Plan 180 Stage K's ledger drain, so K copies two base images, not five. |
| 3 | [181](plans/plan_181_rule_readers_are_code.md) | The rule readers are code | Stage A — a test module defines tests, and the rest is a package | **Y** | -- | 85 | XL | Nothing blocks Stage A. Wants to land before Plan 180 Stage B, which writes a border guard per seam — each one a new reader, and the directory those are written into is decided by whether Stage A exists. Safe stopping point after Stage A: the package, its coverage entry and the boundary rule stop new unmeasured code being added, with nothing yet moved. |
| 4 | [182](plans/plan_182_what_a_unit_owes_itself.md) | What a unit owes itself | Stage A — the within-party frame written into the contract | **Y** | -- | 85 | XL | Nothing blocks Stage A. Stage B wants Plan 181 Stage A's package for its reader; Stage F wants Plan 180 Stage A's double builder. Absorbs Plan 180 Stages J and L as its G and H. Safe stopping point after Stage D: the frame, both ledgers and the mutation harness stand with one module converted |
| 5 | [134](plans/plan_134_archiver_endpoint_failure_contract.md) | Archiver endpoint failure contract | Stage C's exit — deploy 3's 48h gate, and a production page from `hourly_analytics_refresh` naming a failed task and quoting its `failure_reason`. Deploy 1 (`/compact/silver/run`) live 2026-09-07 19:25Z and deploy 2 (`/flush/staging/run`) live 2026-09-10 19:16Z, both gates clean; deploy 3 (`/flush/silver/run`) live 2026-09-12 18:35Z | **N** | Soak until 2026-09-14 18:35Z | 88 | S | Stage B's observation window closed clean 2026-09-07. A clean soak does not supply the page — that takes a real failure or a deliberate one. Stage D follows once Stage C has held a week |
| 6 | [184](plans/plan_184_replace_minio_with_garage.md) | Replace MinIO with Garage | Stage A — Garage proven against our S3 surface | **N** | Plans 180–182 | 82 | XL | Waits behind Plans 180–182 by decision; must land before Plan 125's production Iceberg writes; Garage is vendored through Plan 183. |
...
[**completed_plans.md**](planning/completed_plans.md) — 126 rows, newest first, one row
