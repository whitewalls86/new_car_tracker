...
def archive_artifacts(
    ...
    Processes in chunks of CHUNK_SIZE to bound memory usage — each chunk does its
    ...
    try:
        for i in range(0, len(artifact_ids), CHUNK_SIZE):
            chunk_ids = artifact_ids[i:i + CHUNK_SIZE]

            # 1. Fetch metadata for this chunk only
            ...
            # 3. Write parquet and commit archived_at
    ...
    return [results.get(aid, {"artifact_id": aid, "archived": False, "reason": "not_processed"})
